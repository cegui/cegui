/************************************************************************
	...
	purpose:	Implements FactoryModule for Mac systems
*************************************************************************/
/*************************************************************************
...
FactoryModule::FactoryModule(const String& filename)
{
	...
	data->handle = mac_loadExeBundle(filename.c_str());

	// check for library load failure
	if (data->handle == NULL)
	{
        const char* error = mac_errorBundle();
		throw	GenericException(
            (utf8*)"FactoryModule::FactoryModule - Failed to load module '" + 
            filename + "' " + error + ".");
	}

	data->regFunc = (ModuleDat::FactoryRegFunc)mac_getBundleSym(data->handle, "registerFactory");

	// check for failure to find required function export
	if (data->regFunc == NULL)
	{
        const char* error = mac_errorBundle();
		mac_unloadExeBundle(data->handle);
		delete data;
...
{
	mac_unloadExeBundle(((ModuleDat*)d_data)->handle);
