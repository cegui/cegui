...
# internal function used to replace the 'real' ogre check.  This one is used to
# enable building of the renderer module code which we adopted for the
# RenderTarget renderer modules rewrite.
AC_DEFUN([CEGUI_CHECK_OGRE_BUILD], [
    PKG_CHECK_MODULES(OGRE, OGRE >= 1.4.6, [cegui_found_ogre=yes], [cegui_found_ogre=no])
    PKG_CHECK_MODULES(OIS, OIS >= 1.0.0, [ois_found=yes],[ois_found=no])

    AC_ARG_ENABLE([ogre-renderer], AC_HELP_STRING([--disable-ogre-renderer], [Disable building the Ogre renderer]),
        [cegui_enable_ogre_build=$enableval],[cegui_enable_ogre_build=yes])
    ...
    if test x$cegui_found_ogre = xyes && test x$cegui_enable_ogre_build = xyes; then
        cegui_building_ogre_renderer=yes
        AC_MSG_NOTICE([Building of Ogre3D renderer module is enabled.])
    else
        cegui_building_ogre_renderer=no
        AC_MSG_NOTICE([Building of Ogre3D renderer module is disabled.])
    fi

    if test x$cegui_building_ogre_renderer = xyes && test x$ois_found = xyes; then
    ...
    AM_CONDITIONAL([BUILD_OGRE_RENDERER], [test x$cegui_building_ogre_renderer = xyes])
