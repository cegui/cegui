...
void LuaScriptModule::executeScriptFile(const String& filename, const String& resourceGroup)
	...
	int error =	luaL_loadbuffer(d_state, (char*)raw.getDataPtr(), raw.getSize(), filename.c_str()) ||
				lua_pcall(d_state,0,0,0);
...
void LuaScriptModule::executeString(const String& str)
	{
	// load code into lua and call it
	int error =	luaL_loadbuffer(d_state, str.c_str(), str.length(), "cegui_execute_string") ||
				lua_pcall(d_state,0,0,0);
